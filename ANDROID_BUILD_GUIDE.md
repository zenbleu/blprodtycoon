...
2. Rebuild: `pnpm run build && pnpm run cap:sync`
3. Clear build cache: `cd android && ./gradlew clean`

### ❌ "pnpm not found" or dependency errors
**Solution:**
...
├── dist/                   (Built web files - generated by pnpm run build)
...
5. **Run `pnpm run build` to ensure web build works**
